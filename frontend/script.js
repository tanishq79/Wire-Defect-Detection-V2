...
async function readApiError(res) {
    ...
    if (!responseText) return `Request failed (${res.status})`;
    try {
...
async function setMachineFromSettings() {
    ...
    if (!Number.isInteger(requested) || requested < 100 || requested > 900 || requested % 100 !== 0) {
        setText("cfg-machineStatus", "Select 100, 200, ..., or 900.");
        input.focus();
        return;
    }
    try {
...
function reconnectAfterSoftwareUpdate() {
    ...
    const retry = async () => {
        ...
        if (Date.now() - startedAt < 120000) {
            ...
            status.textContent = "Restart is taking longer than expected. Check inspection_data/software_update.log.";
        }
    };
